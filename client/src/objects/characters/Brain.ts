...
export default class Brain {
	...
	private processTargetDetection(obj: AutomatedCharacter): void {
		...
		this._targets.forEach((target: Player): void => {
			...
			if (
				x + w >= leftRange &&
				x + w <= rightRange &&
				target.y + target.h >= attacker.y &&
				target.y <= attacker.y + attacker.h
			) {
	...
	private processMovement(item: AutomatedCharacter): void {
		...
		const endMovingPointLeft: boolean =
			obj.x <= obj.movementPoints.startX - obj.movementPoints.length;
		const endMovingPointRight: boolean =
			obj.x >= obj.movementPoints.startX + obj.movementPoints.length;
